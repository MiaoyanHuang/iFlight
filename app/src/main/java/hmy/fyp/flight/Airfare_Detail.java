...
public class Airfare_Detail extends AppCompatActivity {
    ...
    private void displayDetail() {
        ...
        String TotalDuration = items.getLegs().get(0).getDurationInMinutes();
        ...
        airfare_detail_totalDuration.setText(transformDuration(TotalDuration));

        // For Transfer Flight
        if(items.getLegs().get(0).getSegments().size()>1 && !items.getLegs().get(0).getStopCount().equals("0")) {
            ...
            // Calculate Wait Time
            String duration_1st = items.getLegs().get(0).getSegments().get(0).getDurationInMinutes();
            String duration_2nd = items.getLegs().get(0).getSegments().get(1).getDurationInMinutes();
            String WaitTime = String.valueOf(Integer.parseInt(TotalDuration) - Integer.parseInt(duration_1st) - Integer.parseInt(duration_2nd));
    ...
    public String compareDate(String departureDate,String arrivalDate){
        ...
        Date date2;
        try {
            date1 = dateFormat.parse(departureDate);
            date2 = dateFormat.parse(arrivalDate);
        } catch (ParseException e) {
            throw new RuntimeException(e);
        }
        String days = String.valueOf(Objects.requireNonNull(date2).compareTo(date1));
        if (days.equals("0")){
            return "";
        }else {
            return "+" + days;
