...
import com.example.flight.adapter.Adapter_Airfare;
import com.example.flight.bean.airfare.Airfare_Bean;
import com.example.flight.bean.airfare.itineraries.Buckets;
...
public class Airfare_Result extends AppCompatActivity {
    ...
    private void initView() {  //后续考虑开发按钮被按下的反馈
        ...
        airfare_result_best.setOnClickListener(v -> displayAirfare(0, airfare_result_best, "Best"));

        //Cheap Button Listener
        airfare_result_cheap.setOnClickListener(v -> displayAirfare(1, airfare_result_cheap,"Cheapest"));

        //Fast Button Listener
        airfare_result_fast.setOnClickListener(v -> displayAirfare(2, airfare_result_fast,"Fastest"));

        //Direct Button Listener
        airfare_result_direct.setOnClickListener(v -> displayAirfare(3, airfare_result_direct,"Direct"));

        displayAirfare(0, airfare_result_best, "Best"); // Display default Information when redirect to result
    }
    ...
    private void Gson_Airfare(){
        ...
        buckets = airfare_bean.getItineraries().getBuckets();
        Log.d(TAG, "buckets size: " + buckets.size());
    ...
    private void displayJourneyDetails(String dep_AirportName, String arr_AirportName, String date) { //Display flight details
        ...
        for (int i = 0; i < buckets.size(); i++) {
            ResultCount += buckets.get(i).getItems().size();
        }
        airfare_result_totalResult.setText(String.valueOf(ResultCount));
    }

    /**
     * Function: Display Airfare Information
     */
    private void displayAirfare(int array, Button buttonName, String bucketsName) {
        setButtonProperty(buttonName);
        airfare_result_noDataIcon.setVisibility(View.INVISIBLE);
        if (buckets.size() -1 >= array && buckets.get(array).getName().equals(bucketsName)) {
            List<Items> items = buckets.get(array).getItems();
            if (items != null) {
                Log.d(TAG, "buckets: " + items);
                Adapter_Airfare adapter_airfare = new Adapter_Airfare(this, items);
                ...
                //Airfare List Item Click Listener
                adapter_airfare.setOnItemClickListener(position -> redirectToDetail(items.get(position)));
            } else {
