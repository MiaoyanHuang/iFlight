...
import hmy.fyp.flight.picker.Picker_Airport;
...
public class Airfare extends AppCompatActivity {
     ...
     */
    private final Handler handler_airfare = new Handler(Objects.requireNonNull(Looper.myLooper())) {
        @Override
    ...
    private void intiView() {
        ...
        airfare_date_input.setOnTouchListener((v, event) -> {
            if (event.getAction() == MotionEvent.ACTION_DOWN) {
                ...
                datePickerDialog.getDatePicker().setMaxDate(calendar.getTimeInMillis()); // Set the maximum date to 10 months later
